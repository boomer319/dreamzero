"""3-view variant: replay training frames to the DreamZero ZMQ inference
server (socket_test_g1_dex3_3view.py, AgiBot-block geometry: num_frames=9,
action_horizon=48, num_action_per_block=48) and score the returned 48-step
action chunks against the dataset's ground-truth actions.
...
on the sent state vector (1 state row per call); the returned actions are
absolute (the server denormalizes and adds the sent state back). So for
anchor a_j = start + 48*j we send frame a_j + state[a_j] and expect
actions ~= action[a_j : a_j+48]. 3 cameras (bottom-right grid view is black).
...
def parse_args():
    ...
    p.add_argument("--dataset", default="/datasets/G1_Dex3_GraspSquare_Dataset_GEAR")
...
def load_episode_data(dataset, episode):
    ...
    return {
        ...
        "action": action,
        "length": len(state),
        "prompt": tasks.get(ep.get("task_index", 0), (ep.get("tasks") or [""])[0]),
...
def run_gt(args, call, rid, outdir, metadata):
    ...
    for j, a in enumerate(anchors):
        ...
        black = np.zeros_like(fr[CAM_KEYS[0]])
        nat = np.concatenate([
            np.concatenate([fr[CAM_KEYS[0]], fr[CAM_KEYS[1]]], axis=1),
            np.concatenate([fr[CAM_KEYS[2]], black], axis=1),
        ], axis=0)
