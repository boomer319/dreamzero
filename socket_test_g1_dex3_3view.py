...
class G1Dex3Server:
    ...
    def _build_observation(self, obs: dict) -> dict:
        ...
        # The client controls the frame count; the replay protocol sends T=1 per
        # call (cold-start I2V from the sent frame). T>1 is not supported by the
        # real-world-execution VAE branch for this geometry.
        ...
        for obs_key, model_key in image_key_mapping.items():
            if obs_key in obs:
                ...
                h, w = 176, 320
                converted[model_key] = np.zeros((1, h, w, 3), dtype=np.uint8)

        # One state row per call: the model's per-block register holds exactly
        # num_state_per_block=1 state token (AgiBot block geometry).
        state_keys = [
            "state.left_arm_pos",
            "state.right_arm_pos",
            "state.left_hand_pos",
